...
func (b *Backend) BaseFee(blockRes *tmrpctypes.ResultBlockResults) (*big.Int, error) {
	...
	if err != nil || res.BaseFee == nil {
		...
		for i := len(blockRes.BeginBlockEvents) - 1; i >= 0; i-- {
			v2x := blockRes.BeginBlockEvents[i]
			if v2x.Type == feemarkettypes.EventTypeFeeMarket && len(v2x.Attributes) > 0 {
				baseFee, err := strconv.ParseInt(string(v2x.Attributes[0].Value), 10, 64)
				if err == nil {
